...
object ReactBridgeComponent {
  ...
  private def propsObject(c: Context): c.Expr[js.Object] = {
    import c.universe._
    c.Expr(q"_root_.io.kinoplan.scalajs.react.bridge.ReactBridgeComponent.propsToDynamic(${computeParams(c)})")
  }
    ...
    val props = {
      ...
      val convertedProps = params.map { param =>
        ...
        val converted = {
          ...
          if (conv.isEmpty) c.error(c.enclosingPosition, s"Could not find an implicit JsWriter[$paramType]")
